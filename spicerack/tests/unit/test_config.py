...
def test_get_config_empty():
    ...
    config_dir = get_fixture_path(os.path.join('config', 'empty'))
...
def test_get_config_invalid_raise():
    ...
    config_dir = get_fixture_path(os.path.join('config', 'invalid'))
...
def test_get_config_missing():
    ...
    config_dir = get_fixture_path(os.path.join('config', 'non-existent'))
    with pytest.raises(Exception):
        config.get_config(config_dir)
    # TODO: add caplog to test log output once https://github.com/pytest-dev/pytest/issues/3099 is fixed
...
def test_get_config_valid():
    ...
    config_dir = get_fixture_path(os.path.join('config', 'valid'))
    config_dict = config.get_config(config_dir)
...
def test_get_global_config():
    ...
    with mock.patch('spicerack.config.SPICERACK_CONFIG_DIR', get_fixture_path(os.path.join('config', 'valid'))):
